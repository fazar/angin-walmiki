...
.factory('tagService', function($q, $timeout){
  ...
  var searchTags = function(searchFilter) {
    ...
    var matches = tags.filter( function(tag) {
      if(tag.name.toLowerCase().indexOf(searchFilter.toLowerCase()) !== -1 ){return true;}
    });
    $timeout( function(){
